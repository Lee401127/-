...
EncryptionDecryption::EncryptionDecryption(char encryptionKey):crc_table_(256),
      table_generated_(false),
      encryption_key_(encryptionKey) {
    GenerateCrcTable();
}

void EncryptionDecryption::GenerateCrcTable() {
...
uint32_t EncryptionDecryption::CalculateCrc32(const void* data, size_t length) const {
...
void EncryptionDecryption::XorCipher(char *data, size_t length) const
{
    for (size_t i = 0; i < length; ++i) {
...
void EncryptionDecryption::GenerateChecksum(char* data, size_t len) {
...
bool EncryptionDecryption::ValidateData(const char* data, size_t len) {
    if (!table_generated_) GenerateCrcTable();
